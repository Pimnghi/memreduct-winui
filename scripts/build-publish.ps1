...
$nativeProject = Join-Path $projectRoot "src\MemReduct.WinUI.Native\MemReduct.WinUI.Native.vcxproj"
$cliProject = Join-Path $projectRoot "src\MemReduct.WinUI.Cli\MemReduct.WinUI.Cli.vcxproj"
$managedProject = Join-Path $projectRoot "src\MemReduct.WinUI\MemReduct.WinUI.csproj"
...
if ($nativeVersionMatch.Groups[1].Value -ne $managedVersionMatch.Groups[1].Value) {
    throw "Version mismatch: src\MemReduct.WinUI.Shared\app.h is '$($nativeVersionMatch.Groups[1].Value)', managed project is '$($managedVersionMatch.Groups[1].Value)'."
}

$msbuildPath = Get-MsBuildPath
foreach ($architecture in $Platform) {
    Write-Host "Building MemReduct.WinUI.Native ($Configuration|$architecture)..."
    ...
    $nativeDll = Join-Path $projectRoot "src\MemReduct.WinUI.Native\bin\$architecture\CoreLib.dll"
    Assert-PeMachine -Path $nativeDll -Architecture $architecture

    Write-Host "Building MemReduct.WinUI.Cli ($Configuration|$architecture)..."
    ...
    $cliExecutable = Join-Path $projectRoot "src\MemReduct.WinUI.Cli\bin\$architecture\mrw-cli.exe"
