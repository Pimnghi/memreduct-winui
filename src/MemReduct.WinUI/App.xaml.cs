...
public partial class App : Application
{
    ...
    protected override void OnLaunched(Microsoft.UI.Xaml.LaunchActivatedEventArgs args)
    {
            ...
            || (isClean && isAutostart))
        {
            ExitCommandLine(
                pipeName,
                Core.CoreService.GetString(Core.StrId.CommandLineUsage)
        ...
        {
#if !DEBUG
            if (!Core.CoreService.IsElevated())
            {
                if (pipeName == null)
            ...
            var mask = isFullClean
                ? Core.MemoryMask.All
                : Core.IniConfig.ReadUInt("ReductMask2", Core.MemoryMask.Default);
            var result = System.Threading.Tasks.Task
                .Run(() => Core.CleanupCoordinator.CleanAsync(
                    Core.CleanupSource.CommandLine,
                    mask))
                .GetAwaiter()
                .GetResult();

            if (result?.Status == Core.CleanupStatus.Success)
            {
                ExitCommandLine(
                    pipeName,
                    Core.CoreService.FormatCleanedMessage(result.FreedFormatted),
                    0,
                    isError: false);
            }
            else
            {
                string message;
                if (result?.Status == Core.CleanupStatus.PartialSuccess)
                {
                    message = Core.CoreService.FormatPartialCleanupMessage(result);
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        var title = Core.CoreService.GetString(
                            Core.StrId.CleaningFailed) ?? "Memory cleaning failed";
                        message =
                            $"{title}. {Core.CoreService.FormatFailedAreasMessage(failedMask)}";
                    }
                }
        ...
        Core.ToastService.Initialize();
        Core.TrayIcon.SetIcon(System.IO.Path.Combine(AppContext.BaseDirectory, "Assets", "AppIcon.ico"));
        Core.TrayIcon.Create("Mem Reduct WinUI");
        ...
        if (Core.IniConfig.ReadBool("IsStartMinimized") || isAutostart)
            MainWindow.AppWindow.Hide();

        Core.AutoCleanService.Refresh();
        Core.AutoStartService.EnsureConfigured();
    ...
    {
        if (theme == null) theme = Core.IniConfig.ReadString("Theme", "System") ?? "System";
    ...
    {
        var title = Core.CoreService.GetString(
            Core.StrId.CommandLineHelpTitle)
            ?? "Mem Reduct WinUI command-line tool";
        var showHelp = Core.CoreService.GetString(
            Core.StrId.CommandLineHelpShow)
            ?? "Show this help information.";
        var clean = Core.CoreService.GetString(
            Core.StrId.CommandLineHelpClean)
            ?? "Clean memory using the areas selected in the current configuration.";
        var cleanFull = Core.CoreService.GetString(
            Core.StrId.CommandLineHelpFull)
